...
object TranslateQuote {
  def apply(quote: Quote, lang: String = TranslationChain.defaultLang, chain: Seq[String] =  TranslationChain.defaultChain)(implicit as: ActorSystem, ex: ExecutionContext): Future[TranslatedQuote] = {
  ...
  def applyTranslation(quote: Quote, translation: Map[String, String]): Quote =
    quote.copy(statements = quote.statements
      .map{
        statement =>
statement.copy(items =
        statement.items.map{
